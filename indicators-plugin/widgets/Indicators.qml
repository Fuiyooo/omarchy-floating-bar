...
BarWidget {
  ...
  // Inactive indicators extend LEFT of the active block while the centre of
  // the bar is hovered (the bar's own center-section reveal). Relying on a
  // precise mouse-over of the widget would collapse the block as soon as the
  // pointer drifts after a click, which read as the buttons "disappearing".
