...
Item {
  ...
  component BarPanel: PanelWindow {
    ...
    // Floating layout: inset from screen edges on every mounted side; the
    // hidden parking offset now also has to cancel the floating margin.
    margins {
      top: root.position === "top"
        ? (root.barHidden ? -(root.barSize + root.floatingMargin) : root.floatingMargin) : 0
      bottom: root.position === "bottom"
        ? (root.barHidden ? -(root.barSize + root.floatingMargin) : root.floatingMargin) : 0
      left: (root.position === "left" || !root.vertical)
        ? (root.position === "left" && root.barHidden
          ? -(root.barSize + root.floatingMargin) : root.floatingMargin) : 0
      right: (root.position === "right" || !root.vertical)
        ? (root.position === "right" && root.barHidden
          ? -(root.barSize + root.floatingMargin) : root.floatingMargin) : 0
    }
  ...
  // Partisi entri sebuah section berurutan: entri bertetangga yang punya
  ...
  function groupSegments(entries) {
    ...
    for (var i = 0; i < entries.length; i++) {
      var e = entries[i]
      var g = e && typeof e.group === "string" ? e.group : ""
      if (!current || g === "" || current.group !== g) {
        current = { group: g, entries: [] }
        out.push(current)
      }
  ...
  component ModuleList: Loader {
    ...
    // Consecutive entries sharing the same `group` render inside one pill.
    readonly property var segments: root.groupSegments(entries)
  ...
  // One rendered `group` cluster (or a bare standalone run of ungrouped
  // entries). Purely visual: module slots, click targets, drag/drop geometry
  // and popup anchoring all keep working because they map through window
  // coordinates, which this extra nesting layer does not disturb.
  component GroupPill: Item {
    ...
    Loader {
      ...
      sourceComponent: root.vertical ? segmentColumn : segmentRow
    }
    ...
    Component {
      ...
      Row {
        Repeater {
          model: pill.segment ? pill.segment.entries : []
    ...
    // Capsules stay exactly one bar-cell tall/wide and center their content,
    // so a padded group can never bleed past the bar surface.
    implicitWidth: root.vertical
      ? root.barSize
      : (content.item ? content.item.implicitWidth : 0) + pill.pad * 2
    implicitHeight: root.vertical
      ? (content.item ? content.item.implicitHeight : 0) + pill.pad * 2
      : root.barSize
    width: implicitWidth
